...
class Envir:
    ...
    def wirte_info(self, vl, vr, theta):
...
class Robot:
    def __init__(self, startpos, robot_img, width):
    ...
    def move(self, event=None):
        ...
        # se min speed
        self.vr = min(self.vr, self.min_speed)
        self.vl = min(self.vl, self.min_speed)
...
# the envir
environment = Envir(dmin)

# the robot
robot = Robot(start, r"C:\Main_study\robot\img.png", 0.01 * 3779.52)
...
while running:
    ...
    robot.move()
    environment.wirte_info(int(robot.vl), int(robot.vr), robot.theta)
